...
| `src/loader/pe32_process.c`, `src/loader/pe32_process.h` | PE32 argv/envp page setup, PEB process-parameter/LDR/version wiring, and CRT `.bss` seeding extracted from `pe32_entry.c`. | PE32-only; transition-sensitive setup code. Uses direct syscall helpers and local byte-copy helpers so guest-facing data setup stays libc-independent. |
...
- `src/loader/pe32_process.c` during PE32 process setup
- `src/loader/pe32_run_guest.S`
- `src/loader/gs_base.c`
...
- CRT BSS pre-seeding logic exists in `src/main.c`, `src/crt/crt_mingw.c`, `src/crt/crt_watcom.c`, `src/msvcrt/crt_startup.c`, and `src/loader/pe32_process.c`.
...
- Architecture-dependent pointer writes in `src/loader/teb_peb.c` now use `syscall_safe_guest_write_ptr`; remaining candidates occur in `src/loader/import_table.c`, `src/msvcrt/crt_refptrs.c`, and `src/loader/pe32_process.c`.
...
- `src/loader/pe32_entry.c`: 697 lines after extracting PE32 process setup; standalone PE32 runtime, FS/GS/TLS-sensitive.
...
- Task 03 should explicitly inventory `src/syscall/dispatcher_generated.c`, sample `.exe/.dll`, build `.d` files, and the stray `src/loader/import_resolve.d`.
