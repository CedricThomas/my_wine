...
/*
 * dlmalloc wrapper for my_wine
 * Uses direct syscalls for MMAP/MMUNMAP, avoids libc malloc.
 */

/* ── dlmalloc compile-time options (MUST be before dlmalloc source) ── */
#define USE_DL_PREFIX
#define USE_LOCKS 1
#define MMAP dlmalloc_mmap
#define MMUNMAP dlmalloc_mmunmap
#define dlmalloc_morecore dlmalloc_sbrk_morecore
#define HAVE_MMAP 1
...
/* Override ABORT to use our syscall instead of libc abort() */
...
/* dlmalloc_mmap / dlmalloc_mmunmap via direct syscalls */
static void *dlmalloc_mmap(size_t size) {
    void *ptr = (void *)INLINE_SYSCALL_MMAP(NULL, size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
    return ptr;
}

static int dlmalloc_mmunmap(void *ptr, size_t size) {
    return (int)INLINE_SYSCALL_MUNMAP(ptr, size);
}

/* dlmalloc morecore uses sbrk-style: try to grow the break */
extern void *g_dlmalloc_heap_base;
extern size_t g_dlmalloc_heap_size;

static void *dlmalloc_sbrk_morecore(long size) {
    /* For simplicity, we use mmap for all allocations via MMAP.
       This sbrk fallback returns NULL to force dlmalloc to use mmap. */
    (void)size;
    return (void *)-1;
}

/* Abort via syscall instead of libc abort() */
static void dlmalloc_abort(void) {
    INLINE_SYSCALL_EXIT(127);
}

/* ── Lock implementation for USE_LOCKS=1 ── */
static pthread_mutex_t dlmalloc_mutex = PTHREAD_MUTEX_INITIALIZER;

void dlmalloc_lock(void) { pthread_mutex_lock(&dlmalloc_mutex); }
void dlmalloc_unlock(void) { pthread_mutex_unlock(&dlmalloc_mutex); }


/* ── Begin dlmalloc source (wasi-libc / Doug Lea) ── */
/*
