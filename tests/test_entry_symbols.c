...
static const char *resolve_entry_from_array(
        const char *const *entry_syms,
        const IMAGE_SYMBOL *symbols, int sym_count,
        const char *string_table,
        const IMAGE_SECTION_HEADER *sections, int num_sections)
{
    int si;
    for (si = 0; entry_syms[si] != NULL; si++) {
        uint32_t rva = lookup_symbol_rva(symbols, sym_count, string_table,
...
static void test_mingw_entry_symbols(void)
{
    ...
    check("WinMain variants appear before 'main' in entry_symbols",
          first_winmain_idx >= 0 && main_idx >= 0 && first_winmain_idx < main_idx);
    ...
    check("entry_syms[0] is _WinMain@16", strcmp(entry_syms[0], "_WinMain@16") == 0);
    check("entry_syms[1] is _wWinMain@16", strcmp(entry_syms[1], "_wWinMain@16") == 0);
...
static void test_entry_resolution(const char *path)
{
    ...
    const void *base = mmap(NULL, file_size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
    close(fd);
    if (base == MAP_FAILED) {
    ...
    int sym_count = parse_symbol_table_from_file(path, &nt_headers,
                                                   &symbols, &string_table);

    if (sym_count > 0) {
        /* Resolve entry symbol using the CRT module's entry_syms */
        const char *matched = resolve_entry_from_array(
            entry_syms, symbols, sym_count, string_table,
            sections, num_sections);

        /* Check for WinMain variants in COFF symbols */
        uint32_t winmain_rva = lookup_symbol_rva(symbols, sym_count, string_table,
                                                   sections, num_sections,
                                                   "WinMain@16");
        uint32_t winmain2_rva = lookup_symbol_rva(symbols, sym_count, string_table,
                                                    sections, num_sections,
                                                    "_WinMain@16");
        ...
        if (matched) {
            ...
            printf("  _WinMain@16: %s, WinMain@16: %s, _main: %s\n",
                   ...
                   winmain_rva ? "FOUND" : "absent",
                   main_rva ? "FOUND" : "absent");
        }

        /* For WinMain-based PEs */
        if (strstr(path, "sdl2_window") || strstr(path, "window")) {
            check("WinMain-based PE has WinMain variant (WinMain@16/_WinMain@16/Main@16) in COFF symbols",
...
int main(int argc, char *argv[])
{
    ...
    const char *samples[] = {
        "samples/sdl2_window_32/sdl2_window_32.exe",
