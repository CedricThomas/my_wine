...
/*
 * coff_find_symbol_name — iterate COFF symbols and return index of first match
 * for the given name, or UINT32_MAX if not found.
 *
 * Handles both short names (<=8 chars embedded in symbol) and long names
 * (index into string table). Skips aux symbols as part of iteration.
 */
static uint32_t coff_find_symbol_name(const IMAGE_SYMBOL *syms, uint32_t count,
        const char *str_tab, uint32_t str_len,
        const char *want_name, uint16_t num_sec)
{
    size_t nlen = 0;
    while (want_name[nlen]) nlen++;

    for (uint32_t i = 0; i < count; i++) {
        int match = 0;
        if (syms[i].N.ShortName[0]) {
            if (nlen <= 8 && memcmp(syms[i].N.ShortName, want_name, nlen) == 0
                && syms[i].N.ShortName[nlen] == '\0')
                match = 1;
        } else if (str_len > 0 && syms[i].N.Name.Long < str_len) {
            const char *sn = str_tab + syms[i].N.Name.Long;
            size_t j = 0;
            while (sn[j] && j < nlen) j++;
            if (j == nlen) match = 1;
        }
        if (match) {
            /* Prefer defined symbols — skip imports (SectionNumber==0) and
             * out-of-range sections so coff_lookup_entry finds real code entry.
             * SectionNumber <= 0 means undefined/import; > num_sec is invalid. */
            int sec = syms[i].SectionNumber;
            if (sec > 0 && (uint16_t)sec <= num_sec) {
                return i;
            }
            /* Matched name but not a defined symbol — continue searching */
        }
        if (syms[i].NumberOfAuxSymbols) {
...
/*
 * coff_lookup_entry — parse COFF symbol table and look for D_DoomMain.
 *
 * Opens the PE file, maps it read-only, reads the symbol table from the
 * file (using PointerToSymbolTable as file offset), searches for
 * "D_DoomMain" or "_D_DoomMain", and returns the symbol's RVA.
 * Falls back to pe_entry_rva() if not found or no symbol table exists.
 *
 * Uses only inline syscalls — no glibc malloc/free.
 */
static uint32_t coff_lookup_entry(const char *path, const IMAGE_NT_HEADERS *nt)
{
    uint32_t ptr  = pe_pointer_to_symbol_table(nt);
    uint32_t count = pe_number_of_symbols(nt);
    uint16_t num_sec = pe_section_count(nt);

    if (ptr == 0 || count == 0)
        return pe_entry_rva(nt);
    ...
    struct stat st;
    if (INLINE_SYSCALL_FSTAT(fd, &st) < 0) {
        INLINE_SYSCALL_CLOSE(fd);
        return pe_entry_rva(nt);
    }
    size_t file_size = (size_t)st.st_size;

    void *file = INLINE_SYSCALL_MMAP(NULL, file_size, PROT_READ, MAP_PRIVATE, fd, 0);
    INLINE_SYSCALL_CLOSE(fd);
    if (file == MAP_FAILED)
        return pe_entry_rva(nt);

    size_t sym_size = (size_t)count * IMAGE_SIZEOF_SYMBOL;
    if (ptr + sym_size + 4 > file_size) {
        INLINE_SYSCALL_MUNMAP(file, file_size);
        return pe_entry_rva(nt);
    }

    const IMAGE_SYMBOL *syms = (const IMAGE_SYMBOL *)((const uint8_t *)file + ptr);
    size_t str_off = ptr + sym_size;
    uint32_t str_len = *(const uint32_t *)((const uint8_t *)file + str_off);
    if (str_off + 4 + str_len > file_size)
        str_len = (uint32_t)(file_size - str_off - 4);
    const char *str_tab = (const char *)file + str_off + 4;

    const IMAGE_SECTION_HEADER *secs = get_image_sections((void *)file, nt);

    /*
     * Search order:
     * Search for game-specific entry symbols (e.g. Doom 95).
     * Do NOT search for "_main" or "main" — these are present in ALL
     * MinGW PE32 binaries and would bypass _mainCRTStartup, which
     * performs essential CRT initialization (__getmainargs, global
     * constructors, heap setup).  Standard binaries fall through to
     * pe_entry_rva() which uses the PE AddressOfEntryPoint.
     * 1. "D_DoomMain" / "_D_DoomMain" — custom entry (Doom 95)
     * Falls back to PE entry point RVA if nothing found.
     */
    static const char *const names[] = { "D_DoomMain", "_D_DoomMain", NULL };
    for (int ni = 0; names[ni]; ni++) {
        uint32_t idx = coff_find_symbol_name(syms, count, str_tab, str_len,
                                             names[ni], num_sec);
        if (idx != UINT32_MAX) {
            // coff_find_symbol_name already filters to defined symbols only
            int sec = syms[idx].SectionNumber;
            uint32_t rva = secs[sec - 1].VirtualAddress + syms[idx].Value;
            INLINE_SYSCALL_MUNMAP(file, file_size);
            return rva;
        }
    }
