...
| PE32 runtime entry | `src/loader/pe32_entry.c`, `src/loader/pe32_run_guest.S` | Native 32-bit loader entry, PE32-specific TEB/PEB setup, FS setup, thunk generation, and 32-bit guest jump. | PE32-only. | Glibc allowed before FS/GS-sensitive setup; use direct syscall/custom helpers after FS points at the guest TEB. |
...
| PE32-only | `src/loader/pe32_entry.c`, `src/loader/pe32_run_guest.S`, `src/syscall/clone.S`, `src/syscall/mmap2_asm.S`, `src/heap/musl_malloc_32_compat.c`, PE32 samples. |
