...
my_wine has a small PE DLL loader. `LoadLibraryA` maps a DLL with the same
image-mapping code, applies relocations, resolves imports recursively within a
bounded depth, registers the module, and makes exports visible to
`GetProcAddress`. The PEB LDR lists are populated for loaded modules.

This is still much smaller than Wine's module system:

1. Builtin DLL surface is represented by the static import table.
2. PE DLL loading is sample-oriented and lacks full Windows search/path
   semantics.
3. Export lookup is implemented, but API compatibility depends on local stubs
   and the loaded DLL's own imports being resolvable.
...
my_wine implements the NT syscall handlers listed in `include/nt_syscalls.def`;
the dispatcher switch body is generated by `scripts/gen_dispatcher.py`.
...
| **Synchronization** | `NtCreateEvent`, `NtSetEvent`, `NtResetEvent`, `NtCreateMutex`, `NtReleaseMutex`, `NtCreateSemaphore`, `NtWaitForSingleObject` |
| **Time/performance** | `NtQuerySystemTime`, `NtQueryPerformanceCounter`, `NtQueryPerformanceFrequency`, `NtDelayExecution` |
...
| **Approach** | Full API reimplementation per DLL | Static stub table + sample-oriented PE DLL loading + direct dispatch |
| **Syscall table** | Full — every NT syscall has a handler | Only NT syscalls listed in `include/nt_syscalls.def` |
...
These are active design constraints, not a completed compatibility roadmap.
