...
| PE parsing | `src/pe_headers.c`, `src/pe_imports.c`, `src/pe_rip_scan.c`, `src/pe_symbols.c`, `include/pe*.h` | Parse PE/COFF headers, imports, sections, symbols, RVA conversion, and thunk scan patterns. | Shared PE32/PE32+. | Glibc allowed for setup/tests unless called from a documented guest-sensitive path. |
...
| PE32 runtime entry | `src/loader/pe32_entry.c`, `src/loader/pe32_run_guest.S`, `include/loader/pe32_trampoline.h` | Native 32-bit loader entry, PE32-specific TEB/PEB setup, FS setup, thunk generation, and 32-bit guest jump. | PE32-only. | Glibc allowed before FS/GS-sensitive setup; use direct syscall/custom helpers after FS points at the guest TEB. |
...
| PE32 runtime entry | Shared PE parsing/loader pieces that compile under `MY_WINE32`, PE32 syscall primitives, PE32 heap backend, guest-facing stubs. | PE32+-only entry/trampoline code, musl oldmalloc backend, 64-bit-only ABI assumptions. |
...
| PE32-only | `src/loader/pe32_entry.c`, `src/loader/pe32_run_guest.S`, `src/syscall/clone.S`, `src/syscall/mmap2_asm.S`, `src/heap/musl_malloc_32_compat.c`, `include/loader/pe32_trampoline.h`, PE32 samples. |
| PE32+-only | `src/main.c`, `src/run_guest.S`, `src/syscall/clone64.S`, `src/heap/musl_malloc_wrapper.c`, `src/heap/musl_src/*`, `src/heap/musl_stubs/*`. |
