...
static void module_make_unicode_string(LDR_UNICODE_STRING *us, const char *src)
{
    ...
    syscall_safe_memset(us->Buffer, 0, sizeof(us->Buffer));
...
{
    syscall_safe_memset(g_loader.modules, 0, sizeof(g_loader.modules));
...
loaded_module_t *add_module(void *base, const char *name, IMAGE_NT_HEADERS *nt)
{
    ...
    for (i = 0; i < MAX_MODULES; i++) {
        if (g_loader.modules[i].base == NULL) {
            ...
            syscall_safe_memset(m, 0, sizeof(loaded_module_t));
            m->base = base;
            syscall_safe_copy_str(m->name, name, sizeof(m->name));
            m->nt = nt;
            m->load_count = 1;
            m->ldr_linked = 0;
...
loaded_module_t *find_module_by_name(const char *name)
{
    ...
    for (i = 0; i < g_loader.module_count; i++) {
        if (g_loader.modules[i].base != NULL &&
            syscall_safe_strcasecmp(g_loader.modules[i].name, name) == 0) {
...
void remove_module(loaded_module_t *mod)
{
    ...
    for (i = 0; i < g_loader.module_count; i++) {
        if (&g_loader.modules[i] == mod) {
            syscall_safe_memset(&g_loader.modules[i], 0, sizeof(loaded_module_t));
            g_loader.module_count--;
            return;
        }
    }
