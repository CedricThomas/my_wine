...
static const char k_window_class[] = "DDrawSample32Class";
static const char k_window_title[] = "DDraw Sample 32-bit";
static const char k_window_title_ok[] = "DDraw Sample 32-bit OK";
static const char k_window_title_fail[] = "DDraw Sample 32-bit FAIL";

static void release_palette(IDirectDrawPalette *pal)
{
    if (pal)
        pal->lpVtbl->Release(pal);
}

static void release_surface(IDirectDrawSurface *surf)
{
    if (surf)
        surf->lpVtbl->Release(surf);
}

static void release_ddraw(IDirectDraw *dd)
{
    if (dd)
...
cleanup:
    release_surface(offscreen);
    release_surface(backbuffer);
    release_surface(primary);
    release_palette(palette_from_surface);
    release_palette(palette);
    release_ddraw(dd);
    return hr;
}
