...
static int run_tests(void)
{
    ...
    /* ---- Window ---- */
    printf("  window... ");
    rb_window_t win = rb_window_create("SDL2 Backend Test", -1, -1, 320, 200, RB_WINDOW_SHOWN);
    ...
    if (pal) {
        ...
        T(rb_palette_get_colors(pal, 0, 16, read_colors) == RB_OK, "get_colors failed");
    ...
    if (surf) {
        ...
        T(rb_surface_get_desc(surf, &w, &h, &fmt, &pitch) == RB_OK, "get_desc failed");
        T(w == 320 && h == 200 && fmt == RB_FORMAT_8BIT && pitch == 320, "surface desc mismatch");
    ...
    if (surf2 && surf) {
        T(rb_surface_blt(surf2, NULL, surf, NULL, 0, RB_BLT_SRCCOPY) == RB_OK, "blt SRCCOPY failed");
        ...
        if (data2) {
            ...
            T(rb_surface_blt(surf2, &fill_rect, 0, NULL, 0xFF, RB_BLT_COLORFILL) == RB_OK,
    ...
    if (audio_ok == RB_OK) {
        ...
        if (buf) {
            uint8_t *buf_data;
            uint32_t buf_len;
            T(rb_audio_buffer_lock(buf, 0, 4096, &buf_data, &buf_len) == RB_OK, "audio lock failed");
            T(buf_len > 0, "audio buffer len == 0");
            if (buf_data) memset(buf_data, 0, buf_len);
            T(rb_audio_buffer_unlock(buf, buf_data ? buf_data : (uint8_t *)0, buf_len) == RB_OK,
              "audio unlock failed");
            T(rb_audio_buffer_set_volume(buf, -6000) == RB_OK, "set_volume failed");
            T(rb_audio_buffer_set_pan(buf, -5000) == RB_OK, "set_pan failed");
            T(rb_audio_buffer_set_frequency(buf, 22050) == RB_OK, "set_frequency failed");
    ...
    /* ---- Cleanup ---- */
    printf("  cleanup... ");
    if (surf2) rb_surface_destroy(surf2);
    if (surf) rb_surface_destroy(surf);
    if (pal) rb_palette_destroy(pal);
    rb_shutdown();
    printf("OK\n");
